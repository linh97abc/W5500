...
static OS_STK phy_link_task_stk[WIZCHIP_PHY_LINK_TASK_STK_SIZE];
#endif

static inline void spi_read_write(SPI_TypeDef *spi_dev, SPI_DATA_SETUP *data)
{
...
static inline void GPIO_WriteBit(uint32_t gpio_base, uint32_t pin, enum BitState val)
{
  IOWR_ALTERA_AVALON_PIO_DATA(gpio_base, val << pin);
}

static SPI_TypeDef wiz_spi_inst = {WIZ_SPI_BASE, 0};

SPI_TypeDef *wiz_spi = &wiz_spi_inst;

static OS_EVENT *p_wizchip_spi_mutex;

static void _wizchip_cris_enter(void)
{
  INT8U err;
  OSMutexPend(p_wizchip_spi_mutex, 0, &err);
}

static void _wizchip_cris_exit(void)
{
  OSMutexPost(p_wizchip_spi_mutex);
}
...
{
  INT8U err;
  p_wizchip_spi_mutex = OSMutexCreate(WIZCHIP_SPI_MUTEX_PRIO, &err);
  if (err != OS_ERR_NONE)
  {
    printf("wizchip: Create mutex spi fail\n");
    for (;;)
      ;
  }
...
#ifdef CONFIG_WIZCHIP_PHY_LINK_CHECK
  OSTaskCreateExt(task_wizchip_phy_link,
                  NULL,
                  &phy_link_task_stk[WIZCHIP_PHY_LINK_TASK_STK_SIZE - 1],
                  WIZCHIP_PHY_LINK_TASK_PRIO,
                  WIZCHIP_PHY_LINK_TASK_PRIO,
                  phy_link_task_stk,
                  WIZCHIP_PHY_LINK_TASK_STK_SIZE,
                  NULL,
                  0);
#endif
}
